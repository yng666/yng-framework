...
class File implements LogHandlerInterface
{
    ...
    public function __construct(App $app, $config = [])
    {
        ...
        if (empty($this->config['path'])) {
            $this->config['path'] = $app->getStoragePath() . 'log';
        }
    ...
    protected function checkLogSize(string $destination): void
    {
        if (is_file($destination) && floor($this->config['file_size']) <= filesize($destination)) {
            try {
                rename($destination, dirname($destination) . DIRECTORY_SEPARATOR  . date('Y-m-d H:i:s').'-back'. rand(1,100) . '-' . basename($destination));
            } catch (\Exception $e) {
                //
            }
        }
