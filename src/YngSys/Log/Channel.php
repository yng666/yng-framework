...
use Stringable;
use Yng\Contract\LogHandlerInterface;
use Yng\Event;
use Yng\Event\LogRecord;
...
class Channel implements LoggerInterface
{
    ...
    public function record($msg, string $type = 'info', array $context = [], bool $lazy = true)
    {
        ...
        if (is_string($msg) && !empty($context)) {
            ...
            // 新版格式
            $log_class   = app('Yng\\Log');
            $config_json = $log_class->getChannelConfig($log_class->getDefaultDriver())['json'];
            $msg .= (is_string($context)) ? $context : (($config_json) ? json_encode($context,JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) : var_export($context,true));
        }
