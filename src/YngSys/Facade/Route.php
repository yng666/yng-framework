...
use Yng\Route\Domain;
use Yng\Route\Rule;
use Yng\Route\RuleGroup;
use Yng\Route\RuleItem;
use Yng\Route\RuleName;
 ...
 * @method static \Yng\Route lazy(bool $lazy = true) 设置路由域名及分组（包括资源路由）是否延迟解析
 * @method static \Yng\Route mergeRuleRegex(bool $merge = true) 设置路由域名及分组（包括资源路由）是否合并解析
 ...
 * @method static \Yng\Route pattern(array $pattern) 注册变量规则
 * @method static \Yng\Route option(array $option) 注册路由参数
 ...
 * @method static \Yng\Route bind(string $bind, string $domain = null) 设置路由绑定
 ...
 * @method static \Yng\Route setCrossDomainRule(Rule $rule) 设置跨域有效路由规则
 ...
 * @method static \Yng\Route rest(string|array $name, array|bool $resource = []) rest方法定义和修改
 ...
 * @method static Response dispatch(\Yng\Request $request, Closure|bool $withRoute = true) 路由调度
