...
use Yng\Console\Exception\InvalidOptionException;
use Yng\Console\Output;
use Yng\Framework\Facades\Filesystem;
...
class Make extends Command
{
    ...
    public function controller()
    {
        ...
        if (file_exists($controllerFile)) {
            return $this->writeLine('控制器已经存在!', Output::STYLE_RB);
        }
        ...
        return $this->writeLine("控制器App\\Http\\Controllers{$namespace}\\{$controller}创建成功！", Output::STYLE_GB);
    }

    /**
     * 创建模型
     *
     * @return \Yng\Console\Output
     */
    public function model()
    {
        return $this->writeLine("暂时不支持创建模型！", Output::STYLE_RB);
    }
    ...
    public function middleware()
    {
        ...
        return $this->writeLine("中间件App\\Http\\Middleware{$namespace}\\{$middleware}创建成功！", Output::STYLE_GB);
    }

    public function request()
    {
        return $this->writeLine("暂时不支持创建请求类！", Output::STYLE_RB);
    }
